...
def get_chart(year, month, day, hour, gender_input="女", target_year=2025, is_lunar=False):
    # 1. 時間處理
    if is_lunar:
        lunar_year = year
        lunar_month = month
    ...
    ming_ganzhi = GAN[ming_gan_idx] + ZHI[ming_idx]
    ...
    for i in range(12):
        ...
        palaces.append({
            ...
            "zhi": ZHI[i],
            "tags": {"dayun": is_current_dayun, "liunian": is_current_liunian, "doujun": is_current_doujun}
        })

    date_type = "農曆" if is_lunar else "國曆"
    return {
        "lunar_str": f"{date_type} {lunar_year}年 {lunar_month}月 {lunar_day}日 {ZHI[time_idx]}時 ({year_gan}{year_zhi})",
        "bureau": bureau_name,
        "palaces": palaces,
        "age_info": f"{target_year}年 (虛{current_age}歲)",
        # 🔥 新增這三行原料 🔥
        "year_gan": year_gan,
        "year_zhi": year_zhi,
        "gender": gender_input,
